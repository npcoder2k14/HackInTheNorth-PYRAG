...
class Cricket(object):
    ...
    def list_matches(self):
        ...
        for i in match_list:
            if i.text != "Match":
                team_list.append(i.text.lstrip())
        for i in no_list:
            if i.text !="#":
                nos.append(i.text.lstrip())
        for i in venue_list:
            if i.text!="Venue":
                venue.append(i.text.lstrip())
        for i in result_list:
            if i.text!="Result":
                result.append(i.text.lstrip())
        cnt =len(nos)
        ...
        for i in range(cnt):
            if nos[i]=="1":
                header = head_list[heading].text
                matches[header] = []
                heading = heading+1
            matches[header].append((team_list[i], tour_dates_list[i].text, venue[i], result[i]))
